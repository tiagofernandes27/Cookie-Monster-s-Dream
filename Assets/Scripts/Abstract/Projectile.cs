...
public abstract class Projectile : MonoBehaviour
{
    ...
    public abstract void Launch(Vector2 direction, float speed, float damage);

    protected void FlipVisual(Vector2 direction)
    {
        if (visualSprite == null) return;

        // If direction points left, flip vertically
        visualSprite.flipY = direction.x < 0f;
    }
